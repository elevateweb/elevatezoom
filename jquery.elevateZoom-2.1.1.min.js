...
a.imageSrc+"')"});"inner"==a.options.zoomType&&a.zoomWindow.css({backgroundImage:"url('"+a.imageSrc+"')"});a.$elem.bind("mousemove",function(b){a.setPosition(b)});a.zoomContainer.bind("mousemove",function(b){a.setPosition(b)});"inner"!=a.options.zoomType&&a.zoomLens.bind("mousemove",function(b){a.setPosition(b)});a.options.tint&&a.zoomTint.bind("mousemove",function(b){a.setPosition(b)});"inner"==a.options.zoomType&&a.zoomWindow.bind("mousemove",function(b){a.setPosition(b)});a.zoomContainer.mouseenter(function(){"inner"==
...
this.Eroppos&&(this.windowLeftPos=-1*(this.largeWidth-this.zoomWindow.width()));if("window"==this.options.zoomType)if(1>=this.widthRatio&&(this.windowLeftPos=0),1>=this.heightRatio&&(this.windowTopPos=0),this.largeHeight<this.options.zoomWindowHeight&&(this.windowTopPos=0),this.largeWidth<this.options.zoomWindowWidth&&(this.windowLeftPos=0),this.options.easing)if(b.easing.zoomsmoothmove=function(a,b,c,d,e){return b==e?c+d:d*(-Math.pow(2,-10*b/e)+1)+c},b.browser.mozilla){var d=b.camelCase,c=function(a){return{top:"0px",
bottom:"100%",left:"0px",right:"100%"}[a]||a};b.each(["x","y"],function(a,e){var f=d("background-position-"+e);b.cssHooks[f]={get:function(d){d=b.css(d,"background-position").split(/\s+/,2);return c(d[a])},set:function(d,e){var f=b.css(d,"background-position").split(/\s+/,2);f[a]=c(e);b.style(d,"background-position",f.join(" "))}};b.fx.step[f]=function(a){b.style(a.elem,a.prop,a.now)}});this.zoomWindow.stop().animate({backgroundPositionY:this.windowTopPos,backgroundPositionX:this.windowLeftPos},{queue:!1,
duration:this.options.easingDuration,easing:"zoomsmoothmove"})}else this.zoomWindow.animate({"background-position-x":this.windowLeftPos,"background-position-y":this.windowTopPos},{queue:!1,duration:this.options.easingDuration,easing:"zoomsmoothmove"});else this.zoomWindow.css({backgroundPosition:this.windowLeftPos+"px "+this.windowTopPos+"px"});"inner"==this.options.zoomType&&this.zoomWindow.css({backgroundPosition:this.windowLeftPos+"px "+this.windowTopPos+"px"})},setTintPosition:function(a){this.nzOffset=
...
onComplete:b.noop}})(jQuery,window,document);